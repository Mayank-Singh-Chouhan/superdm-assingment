...
export default function Modal({ isOpen, onClose, children }: ModalProps) {
  ...
  return ReactDOM.createPortal(
    ...
    >
      <FocusLock>
        <div className="bg-fs-border backdrop-blur-lg relative p-6 rounded-xl shadow-lg w-96 max-w-full overflow-y-auto">
          {children}
          <button
            onClick={onClose}
            className="absolute top-2 right-2 rounded bg-fs-background"
          >
